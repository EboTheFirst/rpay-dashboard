...
import { useBranchTransactionFrequencyAnalysis } from '@/hooks/use-branches'
...
interface BranchTransactionFrequencyAnalysisProps {
  branchId: string
  dateFilters?: DateFilters
}

interface FrequencyData {
  summary?: Array<{ metric: string; value: number | string }>
...
export function BranchTransactionFrequencyAnalysis({
  branchId,
  dateFilters = {}
}: BranchTransactionFrequencyAnalysisProps) {
  const { theme } = useTheme()
  const [activeTab, setActiveTab] = useState('summary')

  const { data: frequencyData, isLoading, error } = useBranchTransactionFrequencyAnalysis(
    branchId,
    dateFilters,
    !!branchId
  )
  ...
  if (error) {
    return (
      <Card>
        ...
        <CardContent>
          <div className='flex h-[400px] items-center justify-center'>
            <div className='text-destructive'>Error loading frequency analysis</div>
          </div>
        </CardContent>
      </Card>
    )
  ...
  // Format day data for better display
  const dayData = data.day_of_week?.map(item => ({
    day: item.day_of_week.slice(0, 3), // Abbreviate day names
    count: item.transaction_count
  })) || []
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Analyze transaction patterns by time periods for this branch
        </CardDescription>
      </CardHeader>
      <CardContent>
            ...
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.month_of_year?.map(item => ({
                  month: item.month_of_year.slice(0, 3),
                  count: item.transaction_count
                }))}>
                  <XAxis
                    dataKey="month"
