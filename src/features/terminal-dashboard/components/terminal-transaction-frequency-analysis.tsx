...
// Define the expected data structure (matching agent dashboard pattern)
...
export function TerminalTransactionFrequencyAnalysis({
  ...
  const { data: frequencyData, isLoading, error } = useTerminalTransactionFrequencyAnalysis(
    terminalId,
    dateFilters,
    !!terminalId
  )
  ...
  if (!terminalId) {
    return (
      <Card>
        ...
        <CardContent>
          <div className='flex h-[400px] items-center justify-center'>
            <div className='text-muted-foreground'>Please select a terminal to view frequency analysis</div>
          </div>
        </CardContent>
      </Card>
    )
  ...
  // Format hour data for better display (matching agent dashboard pattern)
  const hourData = data.hour_of_day?.map(item => ({
    hour: `${item.hour_of_day}:00`,
    count: item.transaction_count
  })) || []

  // Day of week data with proper ordering (matching agent dashboard pattern)
